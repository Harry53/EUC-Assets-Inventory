...
- Live-table edits for assets, employees, and software are staged into unprocessed approval tables first; live data changes only after approval.
- Asset Allotment is separated into Joining, Replacement, Rebuild, Exit, Sysadmin Tasks, and Machine Preparation pages.
- Sophos Task tracks machines, unprocessed Sophos requests, machine preparation, exit process, DLP, USB, and remote access approvals with CSV/XLSX upload options.
- Administration includes Notification Config and role/feature access controls.
- DB Administration includes table record counts and additional Sophos predefined queries.
